...
const dataMP = JSON.parse(fs.readFileSync(`./database/mercado pago.json`));
const hours2 = (formato) => {moment.locale("pt")
    ...
    try {
      this.response = await requests.requests({
        ...
        headers: {
          ...
          'X-Idempotency-Key': hours2("DDMMYYHHmmss") // Substitua 'your-unique-id' por um ID único para cada requisição
        },
        json: {
